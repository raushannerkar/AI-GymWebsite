...
from website_folder.forms import RegistrationForm, LoginForm, UpdateAccountForm, ResetForm, ResetPasswordForm
from website_folder import app, db, bcrypt, mail
...
def send_mail(user):
    ...
    msg = Message('Password Reset Request', recipients=[user.email], sender='noreply@mm.com')
    msg.body = f'''
...
def resetrequest():
    ...
    if form.validate_on_submit():
        ...
        if user:
            send_mail(user)
            flash('Reset request sent, pls check your mail')
            return redirect(url_for('login'))
...
def reset_token(token):
    ...
    if user is None:
        flash('That is an invalid token or expired. Please try again')
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('login'))
    return render_template('change_password.html', title = "ChangePassword", form = ResetPasswordForm())
...
def contact():
    ...
    msg = Message(subject, sender=('Your Name', 'your_email@example.com'), recipients=['raushnerprojects@gmail.com'])
